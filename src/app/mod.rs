...
/// (`inbox::notify::flush_agent_queue` — draft-state gate, busy/typing holds,
/// MAX_DEFER caps live there so the daemon's per-tick `notification_flush`
/// handler applies the IDENTICAL policy in headless mode). This wrapper only
/// adds the pane-count fast path and refreshes the badge afterwards.
    ...
    F: FnMut(&str) -> anyhow::Result<()>,
{
    ...
    crate::inbox::notify::flush_agent_queue(home, &pane.agent_name, injector);
